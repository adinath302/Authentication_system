...
const SignUp = () => {
  ...
  const Handlesubmit = e => {
    ...
    if (Validateform()) {
      ...
      localStorage.setItem('signupData', 'DummyData')
    }
  }

  return (
    <div className='grid grid-cols-1 xl:grid-cols-[55%_45%] templet-rows-2 h-screen'>
        ...
        <form action='' className='mt-10 gap-10' onSubmit={Handlesubmit}>
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-10'>
            ...
            <label htmlFor='' className=' flex-col gap-1 text-gray-700'>
              Confirm Password:
              <div className='p-2 bg-gray-200 rounded-md flex justify-between items-cen'>
                <input
                  required
                  onChange={event => setConfirmPassword(event.target.value)}
                  value={confirmPassword}
                  type={Ishidden ? <FaEyeSlash /> : <FaEye />}
                  placeholder='xxxxxxxxxx'
